...
const Banner = () => {
    const { videos, categories } = useVideosContext();
    ...
    const bannerVideoCategory = categories.find(category => category.area === bannerVideo.area);
    const categoryColor = bannerVideoCategory.color;

    return (
        <Link to={`/${bannerVideo.id}`}>
            <StyledBanner $cover={bannerVideo.cover}>
                <div>
                    <CategoryTitle color={categoryColor}>{bannerVideo.area}</CategoryTitle>
